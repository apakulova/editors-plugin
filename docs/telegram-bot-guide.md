...
PostHog вернул неожиданный формат данных. Попробуй проверить данные <a href="https://eu.posthog.com/project/184090/dashboard/695809">в полном дашборде</a> (открывается только с vpn)
```
...
Команда `/error_types` пока отложена. Идея: присылать справку по типологии ошибок с редакторским названием, техническим кодом и понятным объяснением. Согласованные наработки лежат в `docs/error-monitoring.md`; сначала нужно реализовать пользовательский UI-отчет об ошибках внутри Figma.
